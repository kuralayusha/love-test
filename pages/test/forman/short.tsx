...
import dataMS from '../../../MS.json'

function ManShort() {
  const [MSquestions, setMSquestions] = useState<any>(dataMS)
  ...
  function handleAnswerClick(point: number, questionId: number) {
    ...
    const questionIndex = MSquestions.questions.findIndex(
  ...
  function handleResultText() {
    if (result === 'no go') {
      ...
      setResultText(
        'oww girl you are so lucky dont lose him this is the one'
      )
    }
  ...
  return (
    <div>
      {showQuestions && (
        <>
          {MSquestions.questions.map((question: any) => (
            <div key={question.id}>
