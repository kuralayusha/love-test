...
function WomanLong() {
  ...
  function handleSolution() {
    if (selectedPoints.length !== 5) {
      alert('please answer all the 5 questions')
    } else {
  ...
  function handleSetResult(pointH: number) {
    if (pointH < 5) {
      setResult('no go')
    } else if (5 <= pointH && pointH < 8 && pointC > 6) {
      setResult('danger')
    } else if (5 <= pointH && pointH < 8 && pointC <= 6) {
      setResult('fun')
  ...
  function handleLeave() {
    ...
    // change all the active classes to false
  }
